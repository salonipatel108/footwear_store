...
let DisplayProducts = () => {
  ...
  if (cart) {
    cart.innerHTML = `Cart ${cartProducts.length}`;
  }
...
const addacart = (id) => {
  ...
  if (ProductInCart) ProductInCart.Quontity += 1;
    ...
    let records = JSON.parse(localStorage.getItem("products")) || [];
    let prd = records.find((e) => e.id == id);
    cartProducts.push({
      id: prd.id,
...
function showProductDetail(id) {
  ...
  let p = records.find((el) => el.id == id);

  let reviewHTML = (p.reviews || [])
    .map((r) => `<p><b>${r.reviewerName}</b>: ${"★".repeat(r.rating)} - ${r.comment}</p>`)
    .join("");

  document.getElementById("productDetail").innerHTML = `
    <div class="row g-4">
      <div class="col-md-5 text-center">
        <img src="${p.images[0]}" class="img-fluid border shadow-sm rounded" style="max-height:400px; object-fit:contain;">
      </div>
      <div class="col-md-7">
        <h3 class="fw-bold">${p.title}</h3>
        <div class="rating mb-2">Rating: ${p.rating} ★</div>
        <p class="price-new">$${p.price}</p>
        <p>${p.description}</p>
